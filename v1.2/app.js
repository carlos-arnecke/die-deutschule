/* die deutSCHule — level test + lead capture (v1.2)
...
(function () {
  ...
  function prior() {
    screen('How much German do you already know?',
      ...
      progress(1, 5, 'about you') + radios('prior', PRIOR, S.prior),
  ...
  function situation() {
    ...
    on('#next', function () {
      ...
      S.startWhen = picked('start');
      if (!S.visa || !S.format || !S.timeSlot || !S.startWhen) {
        return warn('Please answer all four.');
      }
      contact();
    });
    on('#back', goals);
  ...
  var AGES = ['under18', '18-24', '25-34', '35-49', '50+'];
  ...
  function contact() {
    ...
    screen(MODE === 'staff' ? 'Visitor details' : 'Where should we send your plan?',
      ...
      '<fieldset><legend>Age</legend>' +
        radios('age', AGES.map(function (a) { return { value: a, label: a.replace('under18', 'Under 18').replace('50+', '50 or over') }; }), S.ageBand) +
      '</fieldset>' +
      '<div class="grid">' +
  ...
  function result() {
    ...
    if (r.addons.length) {
      html += '<section><h3>Worth adding</h3>' + r.addons.map(function (a) {
        return '<div class="card"><strong>' + esc(a.name) + ' — ' +
          (a.included ? 'included in your visa contract' : eur(a.price)) + '</strong><p>' + esc(a.why) + '</p></div>';
      }).join('') + '</section>';
    }
  ...
  function buildLead(r) {
    return {
      meta: { mode: S.mode, startedAt: S.startedAt, completedAt: new Date().toISOString(), version: 'v1.2' },
      contact: {
